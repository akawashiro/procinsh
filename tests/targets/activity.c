...
int main(void){
    alarm(45);prctl(PR_SET_NAME,"procinsh-act");
    ...
    if(!child){prctl(PR_SET_PDEATHSIG,SIGTERM);if(getppid()==1)return 1;prctl(PR_SET_NAME,"procinsh-actpeer");close(p[1]);close(u[0]);close(tcp[0]);close(udp[0]);char buf[256];int fds[]={p[0],u[1],tcp[1],udp[1]};for(int i=0;i<4;i++){size_t n=0;while(n<sizeof(buf)){int r=read(fds[i],buf+n,sizeof(buf)-n);if(r<=0)return 2;n+=r;}}for(int i=1;i<4;i++){if(recv(fds[i],buf,128,MSG_PEEK|MSG_WAITALL)!=128)return 4;if(recv(fds[i],buf,128,MSG_WAITALL)!=128)return 5;}sleep(10);return 0;}
