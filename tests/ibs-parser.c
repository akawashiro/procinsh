...
int main(void){
    ...
    put(&s,110,&record,sizeof(record));assert(alpha_ibs_poll(&s,out,2,&lost)==1);assert(out[0].pid==42&&out[0].tid==43&&out[0].addr==0x12345000&&out[0].time==100);assert(!lost);
    record.header.misc=PERF_RECORD_MISC_KERNEL;put(&s,0,&record,sizeof(record));assert(alpha_ibs_poll(&s,out,2,&lost)==0);
    struct {struct perf_event_header header;uint64_t id,lost;} lr={{.type=PERF_RECORD_LOST,.size=24},1,7};put(&s,0,&lr,sizeof(lr));assert(alpha_ibs_poll(&s,out,2,&lost)==0&&lost==7);
    record.header.size=0;put(&s,0,&record,sizeof(record));assert(alpha_ibs_poll(&s,out,2,&lost)==0&&lost==8);
    free(s.map);return 0;
}
